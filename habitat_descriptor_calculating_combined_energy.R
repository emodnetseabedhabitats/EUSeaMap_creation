...
#Calculates the overall energy level layer by combining the current-induced energy level layer and the wave-induced energy level layer. 
#The rule is to keep the maximum value of the two layers is kept, e.g. if for a cell the current is moderate and the wave is low, 
#then the combined energy is moderate.  
#When it comes to the confidence, for each cell the confidence is that of the habitat descriptor that contributes to the overall energy level, 
#i.e. if for a cell the combined energy is that of wave (because wave energy level > current energy level), then the combined confidence is that of wave. 
#If for a cell the combined energy level is that of wave and current (because wave energy level = current energy level), the confidence is the average confidence of wave energy level and current energy level, rounded up.
#In addition to the full-coverage energy level confidence raster, a confidence raster specific to EUNIS 2007-11 is created; 
#In this raster only cells where the seabed substrate is rock and the biozone is infralittoral, circalittoral or deep circalittoral are assigned a confidence value 
#(i.e. all other cells are assigned NA).
...
#working directory
workingDirectory<-"D:/temp/EUSeaMap_exercises/partialBoB"
...
#path to the input raster file name corresponding to the biozones. Will be used to create of the EUNIS2007-11 confidence raster 
biozone_raster_fileName<-"output/biozones.tif"

#path to the input raster file name corresponding to the seabed substrate. Will be used to create the EUNIS2007-11 confidence raster 
substrate_raster_fileName<-"input/seabed_substrate.tif"
...
#path  to the output EUNIS2007-11 confidence file

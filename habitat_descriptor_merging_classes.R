...
merge_habitat_descriptor_units<-function(units_to_be_merged,habitatDescriptorShortName) {
  ...
  if (output_probability_raster) {
    ...
    raster_merged_units <- do.call(merge, lst_proba_raster_to_merge)
    outFile<-sprintf("%s_proba.tif",habitatDescriptorShortName)
    print (sprintf("   writing %s",outFile))
    writeRaster(raster_merged_units,file.path("output",outFile),datatype='FLT4S',overwrite=TRUE)
    rm(raster_merged_units,lst_proba_raster_to_merge)
    tmpFiles(remove=T)
    gc()
    }
}
